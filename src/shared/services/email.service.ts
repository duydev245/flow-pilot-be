...
const customerConsultationTemplate = fs.readFileSync(
  path.resolve('src/shared/email-templates/customer_consultation_request.html'),
  'utf-8',
)
const salesConsultationTemplate = fs.readFileSync(
  path.resolve('src/shared/email-templates/sales_consultation_request.html'),
  'utf-8',
)
const paymentRequestTemplate = fs.readFileSync(
  path.resolve('src/shared/email-templates/payment_request_email.html'),
  'utf-8',
)
const paymentSuccessTemplate = fs.readFileSync(
  path.resolve('src/shared/email-templates/payment_success_email.html'),
  'utf-8',
)
const paymentFailureTemplate = fs.readFileSync(
  path.resolve('src/shared/email-templates/payment_failure_email.html'),
  'utf-8',
)
const assignTaskTemplate = fs.readFileSync(path.resolve('src/shared/email-templates/assign_task_email.html'), 'utf-8')
@Injectable()
export class EmailService {
  ...
  async sendSalesConsultationRequestEmail(payload: {
    email: string
    name: string
    phone: string
    company: string
  ...
  async sendPaymentRequestEmail(payload: {
    email: string
    package_name: string
    order_id: string
    payment_id: string
  ...
  async sendAssignTaskEmail(payload: {
    email: string
    name: string
    task_name: string
    project_name: string
    ...
    const subject = `[FLOW-PILOT] NEW TASK ASSIGNED - ${payload.task_name}`

    const formattedDueDate = parse(payload.due_date, 'yyyy-MM-dd', new Date())
    const dueDateString = formattedDueDate.toLocaleDateString('en-GB', {
      day: '2-digit',
      month: '2-digit',
      year: 'numeric',
    })
    ...
    const htmlContent = assignTaskTemplate
      ...
      .replaceAll('{{subject}}', subject)
      .replaceAll('{{name}}', payload.name)
      .replaceAll('{{task_name}}', payload.task_name)
      .replaceAll('{{project_name}}', payload.project_name)
      .replaceAll('{{due_date}}', dueDateString)
